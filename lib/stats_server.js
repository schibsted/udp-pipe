...
function make_stats(stats) {
    ...
    return {
        stats: {
            mean: util.sprintf("%.1f", stats_collection.http.requestsPerSecond.mean),
            count: util.format_number(stats_collection.http.requestsPerSecond.count, 0),
            current_rate: util.sprintf("%.1f", stats_collection.http.requestsPerSecond.currentRate),
            max_rate: util.sprintf("%.1f", stats.max_rate),
            '1MinuteRate': util.sprintf("%.1f", stats_collection.http.requestsPerSecond['1MinuteRate']),
            '5MinuteRate': util.sprintf("%.1f", stats_collection.http.requestsPerSecond['5MinuteRate']),
            '15MinuteRate': util.sprintf("%.1f", stats_collection.http.requestsPerSecond['15MinuteRate']),
