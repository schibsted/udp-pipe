        ...
        try {
            ...
            if (stat.isDirectory()) {
                ...
                fs.readdirSync(path).forEach(function (file) {
                    if (file.match(/\.js$/)) {
                        if (opt.plugins[file] != undefined && opt.plugins[file].disabled != true) {
                            // Setup all plugins.
                            var filename = '.' + path + '/' + file;
                            console.log('Loading plugin: ' + filename);
                            // Load plugins if present
                            var PluginClass = require(filename);
                            plugins[filename] = new PluginClass(opt.plugins[file]);
                            // If plugin has execute method. Attach UDP message event to this method.
