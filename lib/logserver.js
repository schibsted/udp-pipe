...
var Logserver = function (opt, mock_services) {
    ...
    function get_plugin_module (opt, plugin_path, file) {
        ...
        logger.log('Loading plugin: ' + filename);
    ...
    function start_udp_server (opt, stats, callback) {
        ...
        server.on("listening", function () {
            ...
            logger.log("UDP server listening on ".blue + address.address + ":" + address.port);
        });

        server.bind(opt.admin.udp_server_port, function () {
            if (_.isFunction(callback)) {
                var address = server.address();
                logger.log("UDP bind ready on ".blue + address.address + ":" + address.port);
                callback(address);
            }
        });
        logger.log('Started with pid: '.green + process.pid);
    }
    ...
    function send_udp (msg, ip, port) {
        ...
        try {
            client.send(message, 0, message.length, port, ip, function(err, bytes) {
                logger.log('Sent UDP to ' + ip + ':' + port);
                total++;
                client.close();
            });
        } catch (err) {
        ...
        logger.log('Time used: ' + total_time + 's');
    }
