...
var StatsServer = function (opt, mock_services) {
    ...
    function web_socket_server(opt, stats) {
        ...
        logger.log('Listening for websocket connections on port: '.blue +
        ...
        wss.on('connection', function (ws) {
            ...
            logger.log('started websocket client interval'.green);
            // If client connection is lost. Clear interval timer.
            ws.on('close', function () {
                logger.log('stopping websocket client interval'.red);
                clearInterval(id);
            });
        });
    }
    ...
    function web_server(opt, path, stats) {
        ...
        webserver = app.listen(opt.admin.http_server_port);
        logger.log('Webserver for admin client webpage : http://127.0.0.1:'.green +
            opt.admin.http_server_port +
            '/client/'.green);
    }

    return {
        start: function (opt, path, stats) {
